...
make install                 # Symlink ~/.local/bin/pikvm -> ./pikvm (no sudo)
./pikvm.sh --build --test    # Test config.json / .env + tools
./pikvm.sh --help            # All shell commands (build / iso / stream)
```

### Configuration

On first run, `pikvm` expects either:
- `config.json` next to the binary (generated by `automation/scripts/env-from-vault.sh` from HashiCorp Vault), **or**
- `.env` in the same directory with `PIKVM_HOST`, `PIKVM_USER`, `PIKVM_PASS` at minimum.

See [Configuration](#configuration) below for the schema.
